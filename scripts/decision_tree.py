...
def get_data():
    ...
    - y (pd.Series): Binary target labels (1 for birds, 0 for dog_sounds).
    """
...
def perform_decision_tree_classification():
    ...
    plot_and_evaluate_tree(X_train, X_test, y_train, y_test, tree1, "Tree 1: Gini (depth=3, features=10)")
    ...
    plot_and_evaluate_tree(X_train[features], X_test[features], y_train, y_test, tree2, "Tree 2: LogLoss (depth=2, features=5)")
    print("Completed Tree 2")

    #Tree 3: Entropy (depth=3)
    tree3=DecisionTreeClassifier(criterion="entropy", max_depth=3, random_state=18)
    plot_and_evaluate_tree(X_train, X_test, y_train, y_test, tree3, "Tree 3: Entropy (depth=3)")
