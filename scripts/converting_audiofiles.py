...
def convert_audios(category):
    ...
    - Mono audio channel
    - Reduced bit depth (8 bits per sample) for smaller file sizes

    This function:
    - Reads all `.mp3` files in the input directory corresponding to the given `category`.
    - Converts each file to `.wav` format with reduced bit depth and saves it in the corresponding output directory.
...
def convert_audio_helper(file_name,input_path,output_path):
    ...
    - Reduces the bit depth to 8 bits per sample (`-acodec pcm_u8`) to decrease file size.
    ...
    try:
        subprocess.run( #converts to .wav format with sample set rate to 16000Hz, audio channel to mono, reduced bit depth to 8 bits per sample
            ["ffmpeg", "-i", input_path, "-ar", "16000", "-ac", "1", "-acodec", "pcm_u8", output_path], 
